      ...
      fn $set_name() {
        ...
        match fs::stat(&specs_path) {
          Ok(FileStat { size: _, kind: FileType::Directory, .. }) => {
            ...
            match fs::File::open(&Path::new(format!("{}/{}.json", specs_path.as_str().unwrap_or(""), stringify!($set_name)))) {
              Ok(ref mut reader) => {
              ...
              _ => panic!("mustache {} test file not found", stringify!($set_name)),
            }
          },
